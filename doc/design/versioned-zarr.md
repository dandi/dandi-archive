...
Furthermore, though this document has focused on the problems of large Zarrs, Zarr is also a possible backend for NWB files, which generally exist at smaller file sizes. The alternate design shows benefits for this scale as well, providing "copy on write" benefits for individual chunks within a Zarr NWB, taking advantage of chunk deduplication for even better storage scaling than the current system that duplicates whole HDF5 files when an NWB changes.
