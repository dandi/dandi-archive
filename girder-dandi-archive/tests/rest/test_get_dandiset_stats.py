...
def test_get_dandiset_stats_one_folder(server, dandiset_1, request_auth):
    ...
    resp = server.request(
        "/folder",
        method="POST",
        params={"parentType": "folder", "parentId": dandi_id, "name": "Test Folder"},
        **request_auth,
    )
    assertStatusOk(resp)
    resp = server.request(f"/dandi/{identifier}/stats", method="GET")
...
def test_get_dandiset_stats_one_item(server, dandiset_1, request_auth):
    ...
    identifier = dandiset_1["name"]
    resp = server.request(
        "/item", method="POST", params={"folderId": dandi_id, "name": "Test Item"}, **request_auth,
    )
    assertStatusOk(resp)
    resp = server.request(f"/dandi/{identifier}/stats", method="GET")
    assertStatusOk(resp)
...
def test_get_dandiset_stats_tree(server, dandiset_1, request_auth, fsAssetstore):  # noqa: N803
    ...
    def mkfile(name, item_id, content):
        resp = server.request(
            "/file",
            method="POST",
            params={
    ...
    folder_a_id = mkfolder("Folder A", dandi_id)
    folder_b_id = mkfolder("Folder B", folder_a_id)
    folder_c_id = mkfolder("Folder C", folder_b_id)  # noqa: F841
    folder_d_id = mkfolder("Folder D", folder_b_id)

    item_a_id = mkitem("Item A", dandi_id)
    item_b_id = mkitem("Item B", folder_a_id)
    item_c_id = mkitem("Item C", folder_d_id)  # noqa: F841
    ...
    mkfile("test.txt", item_a_id, contents[0])
    mkfile("lorem.txt", item_b_id, contents[1])
    mkfile("jabberwocky.txt", item_b_id, contents[2])
