...
export function useExternalDandisetServices(): Ref<ResolvedDandisetService[]> {
  ...
  watch(
    ...
    () => {
      // Runs before the next re-resolution (or when the watcher stops), so async
      // results landing after the user navigates to another Dandiset (or the
      // instance info loads) are discarded.
      let stale = false;
      onWatcherCleanup(() => {
        stale = true;
      });
