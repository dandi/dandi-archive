<template>
  <v-card v-if="currentDandiset" outlined class="mt-4 px-3">
    <v-dialog v-model="showUploadManagementDialog" max-width="60vw">
      <v-card class="pb-3">
        ...
        <v-card-text>
          This dandiset has <strong>{{ incompleteUploads.length }}</strong> active uploads.
          Unembargo may not proceed until these are addressed. You may delete these uploads using
          the "Clear Uploads" button below.
        </v-card-text>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn depressed @click="showUploadManagementDialog = false"> Close </v-btn>

          <v-menu offset-y bottom>
            <template #activator="{ on, attrs }">
              <v-btn class="ml-2" depressed v-on="on" v-bind="attrs" primary> Clear Uploads </v-btn>
            </template>
    ...
    <v-dialog v-if="showWarningDialog" v-model="showWarningDialog" persistent max-width="60vh">
      <v-card class="pb-3">
        <v-card-title class="text-h5 font-weight-light"> Unembargo Dandiset </v-card-title>
        <v-divider class="my-3" />
        <v-card-text>
          <span>
            This action will unembargo this dandiset. Note that this is a
            <span class="font-weight-bold"> permanent </span>
            action and cannot be undone. Once a dandiset has been unembargoed, it cannot be
            re-embargoed.
            <br /><br />
        ...
        <v-card-text>
          ...
          <v-text-field v-model="confirmationPhrase" style="width: 30%" dense outlined />
        </v-card-text>
        ...
        <v-card-actions>
          ...
          <v-btn depressed @click="showWarningDialog = false"> No, take me back </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const unembargoDisabled = computed(
  () =>
    !!(
      unembargo_in_progress.value ||
      currentDandiset.value === null ||
      currentDandiset.value.active_uploads
    ),
);
