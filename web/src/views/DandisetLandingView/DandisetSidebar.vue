<template>
  <div v-if="currentDandiset">
    <DandisetActions />
    <DandisetOwners :user-can-modify-dandiset="userCanModifyDandiset" />

    <div v-if="currentDandiset.dandiset.embargo_status === 'EMBARGOED'">
      <DandisetUnembargo />
    </div>
    <div v-else>
      <DandisetPublish :user-can-modify-dandiset="userCanModifyDandiset" />
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const otherVersions = computed(() =>
      store.versions?.filter((version: Version) => version.version !== currentVersion.value),
    );
