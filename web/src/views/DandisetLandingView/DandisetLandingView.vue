<template>
  <div>
    ...
    <v-toolbar class="grey darken-2 white--text">
      <DandisetSearchField />
      <v-pagination v-model="page" :length="pages" :total-visible="0" />
    </v-toolbar>
    <v-container
    ...
    >
      <div class="d-block blue-grey lighten-5 pa-4 rounded-lg">
        ...
        <span class="text-body-2">
          ...
          <a href="mailto:help@dandiarchive.org">help@dandiarchive.org</a>.
        </span>
      </div>
    </v-container>
    <v-container
    ...
    <v-container v-else fluid class="grey lighten-4 pa-0">
      <v-progress-linear v-if="!currentDandiset || loading" indeterminate />
      <v-row no-gutters>
        <v-col :cols="$vuetify.breakpoint.smAndDown ? 12 : 10">
          <v-sheet v-if="!currentDandiset || loading" class="py-8 px-7">
            <v-skeleton-loader type="article, list-item, image@2" />
          </v-sheet>
          <DandisetMain v-else :schema="schema" :meta="meta" />
        </v-col>
        <v-col v-if="!$vuetify.breakpoint.smAndDown" cols="2">
          <v-sheet v-if="!currentDandiset || loading" class="py-3">
            <v-skeleton-loader type="card@3" />
          </v-sheet>
          <DandisetSidebar v-else :user-can-modify-dandiset="userCanModifyDandiset" />
        </v-col>
      </v-row>
...
onBeforeRouteLeave((to: Route, from: Route, next: NavigationGuardNext) => {
  ...
  if (
    !editorInterface.value?.transactionTracker?.isModified() ||
