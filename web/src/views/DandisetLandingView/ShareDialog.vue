<template>
  <v-dialog v-if="meta && meta.name" v-model="dialog" offset-y min-width="420" max-width="500">
    <template #activator="{ on }">
      <div class="d-inline" v-on="on">
        <v-icon color="primary" style="cursor: pointer"> mdi-share-variant </v-icon>
        {{ text }}
      </div>
    </template>
    ...
    <v-card>
      ...
      <v-card-text>
        <span class="font-weight-black"> Dandiset link: </span>
        <CopyText
          class="pa-2"
          :text="permalink"
      ...
      <v-card-text v-if="doiLink">
        <span class="font-weight-black"> DOI link: </span>
        <CopyText class="pa-2" :text="doiLink" icon-hover-text="Copy DOI link to clipboard" />
      </v-card-text>
      ...
      <v-card-actions>
        <v-list-item class="grow">
          <v-row align="center">
            ...
            >
              <v-icon class="mr-1" color="blue" large> mdi-twitter </v-icon>
            </ShareNetwork>
          </v-row>
        </v-list-item>
      </v-card-actions>
...
export default defineComponent({
  ...
  setup() {
    ...
    const doiLink = computed(() =>
      currentDandiset.value?.version !== "draft" ? `https://doi.org/${meta.value?.doi}` : "",
    );
