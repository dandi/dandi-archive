<template>
  <v-card v-if="currentDandiset && otherVersions" outlined class="mt-4 px-3">
    <v-dialog
    ...
    >
      <v-card class="pb-3">
        <v-card-title class="text-h5 font-weight-light"> WARNING </v-card-title>
        <v-divider class="my-3" />
        <v-card-text>
          This action will force publish this Dandiset, potentially before the owners are prepared
          to do so.
        </v-card-text>
        <v-card-text> Would you like to proceed? </v-card-text>

        <v-card-actions>
          <v-btn color="error" depressed @click="publish()"> Yes </v-btn>
          <v-btn depressed @click="showPublishWarningDialog = false"> No, take me back </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-row v-if="!publishButtonHidden" no-gutters>
      <v-tooltip :disabled="!publishDisabledMessage" left>
        <template #activator="{ on }">
          <div class="px-1 pt-3" style="width: 100%" v-on="on">
            <v-row
            ...
            >
              ...
              <v-col>
                <span>
                  As an <span class="font-weight-bold">admin</span>, you may publish Dandisets
                  without being an owner.
                </span>
              </v-col>
            </v-row>
            <div class="text-center">
              <v-dialog v-model="showPublishChecklistDialog" width="900">
                ...
                <v-card>
                  <v-card-title class="text-h5 grey lighten-2"> Publish Checklist </v-card-title>

                  <v-card-text>
                    <v-list>
                      <span class="text-body-1 font-weight-bold">
                        For best results, please check the following items before you publish:
                      </span>
                  ...
                  <v-card-actions class="justify-end">
                    <v-btn color="dropzone" depressed @click="showPublishChecklistDialog = false">
                      Cancel
                    </v-btn>
                    <v-btn
                      :color="showPublishWarning ? 'error' : 'success'"
                    ...
                    >
                      Publish
                      <v-spacer />
                      <v-progress-circular v-if="publishing" indeterminate />
    ...
    <v-row v-if="currentDandiset.status === 'Pending'" class="my-2 px-1" no-gutters>
            ...
            <template #activator="{ on: tooltip }">
              ...
              >
                <v-row class="align-center px-4">
                  <v-col cols="1" class="justify-center py-0">
                    <v-icon color="warning" class="mr-1"> mdi-playlist-remove </v-icon>
                  </v-col>
                  <v-spacer />
                  <v-col cols="9">
                    <div v-if="currentDandiset" class="text-caption">
    ...
    >
      <v-row class="align-center px-4">
        <v-col cols="1" class="justify-center py-0">
          <v-icon color="warning" class="mr-1"> mdi-database-remove </v-icon>
        </v-col>
        <v-spacer />
        <v-col cols="9">
    ...
    <v-row>
      <v-subheader class="mb-2 black--text text-h5"> This Version </v-subheader>
    </v-row>
    <v-row
      ...
      "
    >
      <v-col :cols="$vuetify.breakpoint.md ? 12 : 4" style="">
    ...
    <v-row>
      <v-subheader class="mb-2 black--text text-h5"> Other Versions </v-subheader>
    </v-row>
    <v-row
      ...
      "
    >
      <v-col :cols="$vuetify.breakpoint.md ? 12 : 4">
        {{ formatDate(version.modified) }}
      </v-col>
      <v-col>
        <v-btn outlined @click="setVersion(version)">
    ...
    <v-snackbar :value="!!publishedVersion">
      ...
      <template #action="{ attrs }">
        <v-btn color="info lighten-2" text v-bind="attrs" @click="navigateToPublishedVersion">
...
import { onUnmounted, onMounted, computed, ref, watchEffect } from "vue";
...
const otherVersions: ComputedRef<Version[] | undefined> = computed(() =>
  store.versions
    ?.filter((version: Version) => version.version !== currentVersion.value)
    .sort(sortVersions),
);
...
const isOwner: ComputedRef<boolean> = computed(
  () => !!store.owners?.find((owner: User) => owner.username === user.value?.username),
);
