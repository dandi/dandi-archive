<template>
  <div v-if="!unembargo_in_progress">
    <v-progress-linear v-if="!currentDandiset" indeterminate />
    <v-container v-else>
      <v-dialog v-if="!!itemToDelete" v-model="itemToDelete" persistent max-width="60vh">
        <v-card>
          <v-card-title class="text-h5"> Really delete this asset? </v-card-title>

          <v-card-text>
            Are you sure you want to delete asset
            <span class="font-italic">{{ itemToDelete.path }}</span
            >?
          ...
          <v-card-actions>
            <v-spacer />
            <v-btn @click="itemToDelete = null"> Cancel </v-btn>
            <v-btn v-if="itemToDelete" color="error" @click="deleteAsset"> Yes </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
        ...
        <v-col :cols="12">
          <v-card>
            <v-card-title>
              ...
              <v-divider vertical class="ml-2 mr-3" />
              <router-link
            ...
            <v-banner v-else-if="itemsNotFound"> No items found at the specified path. </v-banner>
            <v-list v-else>
              <!-- Extra item to navigate up the tree -->
              <v-list-item v-if="location !== rootDirectory" @click="navigateToParent">
              ...
              >
                <v-icon class="mr-2" color="primary">
                  <template v-if="item.asset === null"> mdi-folder </template>
                  <template v-else> mdi-file </template>
                </v-icon>
                {{ item.name }}
                <v-spacer />

                <v-list-item-action>
                  <v-btn v-if="showDelete(item)" icon @click="setItemToDelete(item)">
                ...
                <v-list-item-action v-if="item.asset">
                  <v-tooltip top>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn icon :href="inlineURI(item.asset.asset_id)" v-bind="attrs" v-on="on">
                ...
                <v-list-item-action v-if="item.asset">
                  <v-tooltip top>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn icon :href="downloadURI(item.asset.asset_id)" v-bind="attrs" v-on="on">
                ...
                <v-list-item-action v-if="item.asset">
                  <v-menu bottom left>
                    ...
                    <v-list v-if="item && item.services" dense>
                      <v-subheader v-if="item.services.length" class="font-weight-medium">
                        EXTERNAL SERVICES
                      </v-subheader>
...
const EXTERNAL_SERVICES = [
...

  {
    ...
    maxsize: Infinity,
    endpoint:
      "https://neurosift.app?p=/nwb&url=$asset_dandi_url$&dandisetId=$dandiset_id$&dandisetVersion=$dandiset_version$", // eslint-disable-line max-len
  },

  {
    ...
    maxsize: Infinity,
    endpoint:
      "https://neurosift.app?p=/nwb&url=$asset_dandi_url$&st=lindi&dandisetId=$dandiset_id$&dandisetVersion=$dandiset_version$", // eslint-disable-line max-len
  },

  {
    ...
    maxsize: Infinity,
    endpoint:
      "https://neurosift.app?p=/avi&url=$asset_dandi_url$&dandisetId=$dandiset_id$&dandisetVersion=$dandiset_version$", // eslint-disable-line max-len
  },
];
...
const unembargo_in_progress = computed(
  () => currentDandiset.value?.dandiset.embargo_status === "UNEMBARGOING",
);
const splitLocation = computed(() => location.value.split("/"));
const isAdmin = computed(() => user.value?.admin || false);
const isOwner = computed(() => !!(user.value && owners.value?.includes(user.value?.username)));
...
function getExternalServices(
  path: AssetPath,
  info: { dandisetId: string; dandisetVersion: string },
) {
  if (path.asset === null) {
    return [];
  }
  ...
  return EXTERNAL_SERVICES.filter((service) => servicePredicate(service, path)).map((service) => ({
    name: service.name,
...
watch(location, () => {
  ...
  if (existingLocation === location.value) {
    return;
  }
  router.push({
    ...route,
  ...
  (newRouteQuery) => {
    location.value =
      (Array.isArray(newRouteQuery.location)
        ...
        : newRouteQuery.location) || rootDirectory;
