...
import { searchResults, loading, page, updateSearchQuery } from "./store";
...
<template>
  <v-container>
    ...
    <v-sheet class="ma-2 pa-2">
      <v-sheet v-if="loading" height="100%" class="d-flex justify-center align-center">
        <v-progress-circular :size="100" :width="5" indeterminate />
      </v-sheet>
      <DandisetList v-else-if="dandisets" class="mx-4 mx-md-8 my-8" :dandisets="dandisets">
        <template v-for="result in dandisets" #[result.dandiset.identifier]>
          <v-sheet :key="result.dandiset.identifier" class="d-flex flex-column">
            <div
