<template>
  <div v-page-title="pageTitle">
    <v-toolbar color="grey darken-2 white--text">
      <v-menu v-if="!user" offset-y :close-on-content-click="false">
        <template #activator="{ on, attrs }">
          <v-icon dark v-bind="attrs" v-on="on"> mdi-cog </v-icon>
        </template>
        <v-list>
          <v-list-item>
          ...
          <v-list-item>
            <v-switch v-model="showDrafts" label="Drafts" dense />
          </v-list-item>
          <v-list-item>
            <v-switch v-model="showEmpty" label="Empty Dandisets" dense />
          </v-list-item>
        </v-list>
      </v-menu>
      <div class="mx-6">Sort By:</div>
      <v-chip-group :value="sortOption" active-class="white light-blue--text" dark mandatory>
        <v-chip v-for="(option, i) in sortingOptions" :key="option.name" @click="changeSort(i)">
          {{ option.name }}
          <v-icon right>
    ...
    />
    <v-container v-else>
      <v-row class="text-center ma-12 grey--text" align="center" justify="center">
        <v-col>
          <v-progress-circular v-if="!dandisets" indeterminate />
