  ...
  <v-row>
    <v-col>
      <v-card outlined height="100%">
        <v-card-title :id="name" class="font-weight-regular">
          <v-icon class="mr-3 grey--text text--lighten-1">
            {{ icon }}
          </v-icon>
          {{ name }}
        </v-card-title>
        <v-list v-if="items && items.length" :style="`column-count: ${columnCount};`" class="px-5">
          <div v-for="(item, i) in items" :key="i">
            <div :class="`my-1 d-inline-block ${backgroundColor}`" style="width: 100%">
              <div
                class="pl-2 my-1 py-1"
                :style="`border-left: medium solid ${borderLeftColor};
                   line-height: 1.25`"
              >
                <v-row no-gutters class="justify-space-between mr-4">
                  <v-col cols="9" class="grey--text text--darken-3">
  ...
  setup(props) {
    ...
    const columnCount = computed(() =>
      $vuetify.value?.breakpoint.mdAndDown
        ? 1
        : Math.min(Math.ceil(props.items.length / 2), MAX_COLUMNS),
    );
