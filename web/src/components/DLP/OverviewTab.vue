<template>
  <div>
    <v-card v-if="contributors && contributors.length" outlined height="100%">
      <v-card-title class="font-weight-regular">
        <v-icon class="mr-3 grey--text text--lighten-1"> mdi-account-multiple </v-icon>
        Contributors
      </v-card-title>
      <div class="px-2 mb-2">
        <v-chip v-for="(contributor, i) in contributors" :key="i" style="margin: 5px" outlined>
          {{ contributor.name }}
          <a
    ...
    <MetadataCard :items="fundingInformation" name="Funding information" icon="mdi-currency-usd">
      <template #content="slotProps">
        <div class="text-caption grey--text text--darken-1">
          <span v-if="slotProps.item.awardNumber" class="pl-2">
            <strong>- Award Number: </strong>{{ slotProps.item.awardNumber }}
          </span>
        </div>
      </template>
      <template #emptyFallback>
        <span class="font-italic font-weight-bold"> No funding information available. </span>
      </template>
    </MetadataCard>
    ...
    >
      <template #content="slotProps">
        ...
        <span v-if="slotProps.item.repository" class="text-caption grey--text text--darken-1">
          <strong>Repo: </strong>{{ slotProps.item.repository }}
          <br />
        </span>
        <span v-if="slotProps.item.relation" class="text-caption grey--text text--darken-1">
    ...
    <v-card v-if="assetSummary" outlined>
      <v-card-title class="font-weight-regular">
        <v-progress-circular
          ...
          indeterminate
        />
        <v-icon v-else class="mr-3 grey--text text--lighten-1"> mdi-clipboard-list </v-icon>
        Assets Summary
      </v-card-title>
      <v-list :style="`column-count: ${assetSummaryColumnCount};`" class="px-3 ml-2">
        <div v-if="assetSummaryBeingComputed" class="text-subtitle-2">
        ...
        <div v-for="([type, items], i) in Object.entries(assetSummary)" v-else :key="i">
          <div v-if="items && items.length" class="d-inline-block" style="width: 100%">
            ...
            >
              <div
                ...
                :style="`border-left: medium solid ${$vuetify.theme.themes.light.primary};
                         line-height: 1.25`"
              >
                <v-row no-gutters class="align-center py-0" style="min-height: 2em">
                  <v-col cols="10">
                ...
                <span v-if="!isURL(item.identifier)" class="text-caption grey--text text--darken-1">
                  {{ item.identifier }}
                </span>
              </div>
            </div>
...
const ASSET_SUMMARY_BLACKLIST = new Set(["numberOfBytes", "numberOfFiles", "schemaKey"]);
...
const contributors = computed(() =>
  props.meta.contributor?.filter((contributor) => !!contributor.includeInCitation),
);
const fundingInformation = computed(
  () =>
    props.meta.contributor?.filter(
      (contributor) =>
        !!(contributor.schemaKey === "Organization") &&
        // Only include organizations with "Sponsor" or "Funder" roles in Funding Information
        (contributor.roleName?.includes("dcite:Funder") ||
          contributor.roleName?.includes("dcite:Sponsor")),
    ) || [],
);
...
const assetSummary = computed<Record<string, any>>(() =>
  Object.fromEntries(
    Object.entries(props.meta.assetsSummary)
      .filter(
...
const assetSummaryColumnCount = computed(() =>
  $vuetify.value?.breakpoint.mdAndDown ? 1 : Math.min(Object.keys(assetSummary.value).length, 3),
);

const assetSummaryBeingComputed = computed<boolean>(
  () => currentDandiset.value?.status === "Pending",
);
