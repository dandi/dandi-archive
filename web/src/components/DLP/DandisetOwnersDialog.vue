<template>
  ...
  >
    <v-card-title class="justify-space-between">
      ...
      <v-btn :small="$vuetify.breakpoint.xs" color="info" elevation="0" @click="save()">
        Done
      </v-btn>
    </v-card-title>
    ...
    <v-row no-gutters>
      <v-col :cols="$vuetify.breakpoint.xs ? 12 : 6" class="d-flex flex-column">
        ...
        <div class="flex-grow-1">
          <div v-for="(result, i) in searchResults" :key="i">
            ...
            <div class="mx-3 px-2" style="width: 100%">
              <div class="d-flex justify-space-between">
                <v-row class="align-center">
                  ...
                  <span v-if="result.name" class="text-caption ml-1 pt-1">
                    {{ result.username }}
                  </span>
                </v-row>
                <v-btn
        ...
        <v-card-actions class="elevation-10 mt-1 px-3 justify-space-between">
          <v-btn class="pa-2" text @click="clearForm"> Clear form </v-btn>
          <v-btn class="pa-2 py-5 grey darken-3 white--text" depressed @click="addSelected">
            <span class="mr-6 ml-2"> Add selected </span>
            <v-icon> mdi-arrow-right </v-icon>
          </v-btn>
        </v-card-actions>
      </v-col>
      <v-col class="grey lighten-3">
        <div class="my-6">
          <v-dialog v-model="selfRemovalWarningDialog" width="40vw">
            <v-card>
              <v-card-title> Remove yourself from this Dandiset? </v-card-title>
              <v-card-subtitle>
                To regain ownership of this dandiset, you will need another owner or an admin to add
                you.
              </v-card-subtitle>
              <v-card-actions>
                <v-spacer />
                <v-btn color="secondary" elevation="0" @click="selfRemovalWarningDialog = false">
                  Cancel
                </v-btn>
                <v-btn color="error" elevation="0" @click="removeOwner(user)"> Confirm </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
          <v-card v-for="(owner, i) in newOwners" :key="i" class="mx-5 pa-3" outlined>
            <div class="d-flex flex-wrap justify-space-between">
              ...
              <span>
                <v-btn v-if="newOwners.length > 1" text small @click="removeOwner(owner)">
                  <v-icon color="error" left>mdi-minus-circle </v-icon>
                  <span class="font-weight-medium"> Remove </span>
                </v-btn>
              </span>
            </div>
          </v-card>
    ...
    <v-dialog v-model="adminWarningDisplay" max-width="60vw" persistent>
      <v-card class="pb-3">
        ...
        <v-card-actions>
          <v-btn color="error" depressed @click="save"> Yes </v-btn>
          <v-btn depressed @click="adminWarningDisplay = false"> No </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-card>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const isSelected = (user: User) =>
      selectedUsers.value.map((u) => u.username).includes(user.username);
    ...
    function checkBoxHandler(_user: User) {
      ...
      if (selectedUsersUsernames.includes(_user.username)) {
        selectedUsers.value = selectedUsers.value.splice(
          selectedUsersUsernames.indexOf(_user.username),
          1,
        );
      } else {
    ...
    async function save() {
      if (currentDandiset.value?.dandiset) {
        const owner = owners.value?.map((u: User) => u.username).includes(user.value!.username);
