...
def test_ingest_zarr_archive_assets(zarr_archive_factory, zarr_file_factory, draft_asset_factory):
    ...
    assert asset.size == 0
    assert asset._populate_metadata()['contentSize'] == 0
    assert asset._populate_metadata()['digest']['dandi:dandi-zarr-checksum'] is None
    ...
    assert asset._populate_metadata()['contentSize'] == zarr_file.size
    assert asset._populate_metadata()['digest']['dandi:dandi-zarr-checksum'] == zarr.checksum
