...
def test_zarr_rest_delete_file_asset_metadata(
    ...
    zarr_archive.refresh_from_db()
    assert asset._populate_metadata()['digest'] == zarr_archive.digest
    assert asset._populate_metadata()['contentSize'] == 100
    ...
    ingest_zarr_archive(zarr_archive.zarr_id)
    asset.refresh_from_db()
    assert asset._populate_metadata()['digest']['dandi:dandi-zarr-checksum'] == EMPTY_CHECKSUM
