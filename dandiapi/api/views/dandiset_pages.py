...
def dandiset_landing_view(request: HttpRequest, identifier: str, version: str = None) -> HttpResponse:
    ...
    try:
        ...
        if version:
            dandiset_version = get_object_or_404(
                Version, 
                dandiset=dandiset, 
                version=version
            )
        ...
        try:
            ...
            if dandiset.embargo_status == 'EMBARGOED':
                ...
                elif not is_dandiset_owner(dandiset, request.user):
                    embargoed_or_unauthenticated = True
...
def _format_size_binary(size_bytes: int) -> str:
    ...
    elif abs(size - round(size)) < 0.01:  # If difference is less than 0.01
