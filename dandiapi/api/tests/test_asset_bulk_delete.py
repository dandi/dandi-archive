...
@pytest.fixture
def version_with_assets(draft_asset_factory) -> tuple[User, Version, list[Asset]]:
...
def test_asset_rest_bulk_delete_partial(api_client, version_with_assets):
    ...
    # Ordering doesn't matter because it's just one asset
...
def test_asset_rest_bulk_delete_asset_of_other_version(
    ...
    """An asset in another version fails identically to if the asset didn't exist."""
...
def test_asset_rest_bulk_delete_published_version(api_client, asset):
    """Test that if any assets of published versions are included, it's rejected."""
    user = UserFactory.create()
