...
from dandiapi.api.models.dandiset import DandiPermissions, Dandiset
...
def change_asset(  # noqa: PLR0913
    *,
    ...
    if not user.has_perm(DandiPermissions.UPDATE_DANDISET_ASSETS, version.dandiset):
...
def add_asset_to_version(
    *,
    ...
    if not user.has_perm(DandiPermissions.CREATE_DANDISET_ASSETS, version.dandiset):
        # TODO: Update exception name
        raise DandisetOwnerRequiredError
    if version.version != 'draft':
        raise DraftDandisetNotModifiableError
...
def remove_asset_from_version(*, user: User, asset: Asset, version: Version) -> Version:
    if not user.has_perm(DandiPermissions.DELETE_DANDISET_ASSETS, version.dandiset):
        # TODO: Update exception name
        raise DandisetOwnerRequiredError
    if version.version != 'draft':
        raise DraftDandisetNotModifiableError
